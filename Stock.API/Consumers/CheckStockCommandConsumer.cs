    ...
    public class CheckStockCommandConsumer : IConsumer<CheckStockCommand>
  {
    ...
    public async Task Consume(ConsumeContext<CheckStockCommand> context)
    {
      ...
      if (stockResult.TrueForAll(sr => sr.Equals(true)))
      {
        ...
        StockReservedEvent reserveStockCommand = new(context.Message.CorrelationId)
        {
          OrderId = context.Message.OrderId
        };
      ...
      {
        StockNotReservedEvent NotReserveStockCommand = new(context.Message.CorrelationId)
        {
